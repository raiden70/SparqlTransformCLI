...
public class QueryTransformer {
    ...
    public void printDataSet() {
        RDFDataMgr.write(System.out, this.dataset, Lang.NQUADS);// this line will print the whole dataset with n-quads
    }
    ...
    public String nary() {
        ...
        for(int i = 0; i<subjects.size();i++)
        {
            for (int j = 0; j <subjects.size() ; j++) {
                ...
                if(predicates.get(i).equals("nary.nt:statementPropertyOf"))
                {
                ...
                if(predicates.get(i).equals("nary.nt:valuePropertyOf"))
                {
                    if(!npredicates.contains(objects.get(i)))
                        npredicates.add(objects.get(i));
